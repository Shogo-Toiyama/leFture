...
enum ActivityType { saved, likes, dislikes, announcements, trash }
...
final trashAnnouncementsProvider = StreamProvider<List<LocalAnnouncement>>((
  ref,
) {
  final uid = supabase.auth.currentUser?.id;
  if (uid == null) return Stream.value(const []);
...
final activityRecordsProvider =
    FutureProvider.family<List<ActivityRecord>, ActivityType>((
      ref,
      type,
    ) async {
      final uid = supabase.auth.currentUser?.id;
      ...
      switch (type) {
            ...
            if (metadata?['saved'] == true) {
              list.add(
                ActivityRecord(
                  ...
                  title: row.keyword.trim().isNotEmpty
                      ? row.keyword.trim()
                      : 'Keyword',
        ...
        case ActivityType.announcements:
          final announcements = await ref.watch(
            allAnnouncementsProvider.future,
          );
        ...
        case ActivityType.trash:
          ...
          final announcements = await ref.watch(
            trashAnnouncementsProvider.future,
          );
...
class TrashController {
  ...
  static Future<void> deleteSingleItem(
    ...
    if (record.type == ActivityRecordType.course) {
      ...
      if (response.statusCode != 200 && response.statusCode != 404) {
        throw Exception(
          'Failed to delete lecture (${response.statusCode}): ${response.body}',
        );
      }
    ...
    final skipLectureIds = {
      ...
      for (final lecture in trashLectures)
        if (lecture.courseId != null &&
            coursesFailed.contains(lecture.courseId))
          lecture.id,
    };
    for (final lecture in trashLectures) {
