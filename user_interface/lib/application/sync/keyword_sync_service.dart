...
/// KeywordのPull(Supabase→ローカルDB)専用サービス。ユーザーによる書き込みが
...
class KeywordSyncService {
  ...
  KeywordSyncService(this._db);

  static const _entityType = 'keyword';
  ...
  Future<void> pull({bool forceFullPull = false}) async {
    ...
    final localCount = await (_db.select(_db.localKeywords)
    ...
    var query = supabase.from('keywords').select().eq('user_id', uid);
    ...
    if (data.isNotEmpty) {
      final companions = data.map((json) {
        ...
        return LocalKeywordsCompanion(
          ...
          definition: Value(json['definition'] as String? ?? ''),
      ...
      await _db.batch((batch) {
        batch.insertAllOnConflictUpdate(_db.localKeywords, companions);
      });

      DevLog.add('📥 [KeywordSync] Pulled ${companions.length} keyword(s) from cloud.');
    }
  ...
  Future<void> pullForLecture(String lectureId) async {
    ...
    DevLog.add('📥 [KeywordSync] Restored ${companions.length} keyword(s) for lecture $lectureId.');
  }
}
