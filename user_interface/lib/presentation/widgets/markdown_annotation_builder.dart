...
String annotationsCacheKey(List<Annotation> annotations) {
  ...
  return sorted
      .map((a) => '${a.id}:${a.startIdx}:${a.endIdx}:${a.annotationType}:${a.contents}')
      .join('|');
}
...
class MarkdownAnnotationBuilder extends MarkdownElementBuilder {
  ...
  Widget? visitElementAfterWithContext(
    ...
    if (_inlineStyles.containsKey(element.tag) && _inlineStyleStack.isNotEmpty) {
  ...
  Widget? visitText(md.Text text, TextStyle? preferredStyle) {
    ...
    for (final a in _annotations) {
      ...
      if (overlapStart > cursor) {
        spans.add(TextSpan(
          text: content.substring(cursor - leafStart, overlapStart - leafStart),
    ...
    if (cursor < leafEnd) {
      spans.add(TextSpan(text: content.substring(cursor - leafStart), style: effectiveStyle));
    }
  ...
  TextStyle _decoratedStyle(TextStyle? base, Annotation a) {
    ...
    final color = parseHexColor(contents is Map ? contents['color'] as String? : null) ??
        Colors.yellow;
