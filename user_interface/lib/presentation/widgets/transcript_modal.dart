// lib/presentation/widgets/transcript_modal.dart
...
/// 互換用エイリアス
@Deprecated('Use showTranscriptModal instead')
...
@Deprecated('Use TranscriptModal instead')
typedef AnnouncementTranscriptModal = TranscriptModal;
...
/// モーダル内でトランスクリプトおよび音声を再生するためのボトムシート。
class TranscriptModal extends HookConsumerWidget {
  ...
  static Future<void> show(
    ...
    return showModalBottomSheet<void>(
      ...
      builder: (context) => TranscriptModal(
        lectureId: lectureId,
        startSid: startSid,
        endSid: endSid,
        highlightSids: highlightSids,
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // iPadではshowModalBottomSheetがMaterial3のデフォルトで幅640ptに制限されるため、
    // MediaQueryのデバイス全幅ではなくLayoutBuilerで実測した幅を使う
      ...
      data: (lecture) {
        ...
        final displayItems = useMemoized(() {
          ...
          for (final m in moments) {
            ...
            sentenceIndexToMoments
                .putIfAbsent(targetSentenceIdx, () => [])
                .add(m);
          }
        ...
        final screenWidth =
            measuredWidth.value ?? MediaQuery.of(context).size.width;
        ...
        useEffect(() {
          final dSub = player.onDurationChanged.listen(
            (d) => duration.value = d,
          );
          final pSub = player.onPositionChanged.listen(
            (p) => position.value = p,
          );
        ...
        final topicRanges = useMemoized(() {
          ...
          for (int i = 0; i < topics.length; i++) {
            ...
            if (endRatio > startRatio) {
              ranges.add(
                TopicProgressRange(startRatio: startRatio, endRatio: endRatio),
              );
            }
          }
          return ranges;
        ...
        return LayoutBuilder(
          builder: (context, constraints) {
            if (measuredWidth.value != constraints.maxWidth) {
              WidgetsBinding.instance.addPostFrameCallback((_) {
                if (measuredWidth.value != constraints.maxWidth) {
                  measuredWidth.value = constraints.maxWidth;
                }
              });
            }
                          ...
                          child: CustomScrollbar(
                            ...
                            child: ListView.builder(
                              ...
                              itemBuilder: (context, idx) {
                                ...
                                final isRangeHighlighted = highlightedIndices
                                    .contains(sIdx);
                                final isHighlighted =
