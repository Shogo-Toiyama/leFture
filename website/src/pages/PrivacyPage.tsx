...
export const PrivacyPage: React.FC = () => {
  ...
  const fetchDocument = async () => {
    ...
    setError(null);
    try {
      const data = await getLegalDocument('privacy_policy', locale);
      setDoc(data);
    } catch (err: any) {
      ...
      <div className="glass-card" style={{ padding: 'clamp(20px, 5vw, 40px)', marginTop: '12px' }}>
        ...
        <div style={{
          ...
          {doc?.effectiveDate && (
            <div style={{
              ...
              <Calendar size={14} />
              <span>
                {t.privacy.effectiveDate}: {new Date(doc.effectiveDate).toLocaleDateString(locale === 'ja' ? 'ja-JP' : 'en-US')}
              </span>
            </div>
          )}
        </div>
        ...
        {loading ? (
          <div style={{
            ...
            <Loader2 size={28} className="animate-spin" color="var(--color-cosmic-blue)" />
            <span style={{ fontSize: '0.95rem' }}>{t.privacy.loading}</span>
          </div>
        ) : error ? (
          <div style={{
            textAlign: 'center',
            ...
            <h3 style={{ fontSize: '1.15rem', marginBottom: '6px' }}>{t.privacy.errorTitle}</h3>
          ...
          <div className="markdown-body" style={{
            ...
            <ReactMarkdown
              components={{
                ...
                h3: ({ children }) => <h4 style={{ fontSize: '1.05rem', margin: '18px 0 8px', color: 'var(--color-cosmic-blue)' }}>{children}</h4>,
